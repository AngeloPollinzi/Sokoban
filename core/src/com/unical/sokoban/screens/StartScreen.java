...
public class StartScreen implements Screen {
	...
	public StartScreen(Sokoban sokoban) {
		this.sokoban = sokoban;
		sokoban.getSounds().play(2);
	...
	public void show() {
		...
		playButton.addListener(new ClickListener() {
			...
			public void clicked(InputEvent event, float x, float y) {
				sokoban.getSounds().stop(2);
				sokoban.getSounds().play(1);
				((Game) Gdx.app.getApplicationListener()).setScreen(new PlayScreen(sokoban));
			}
		});
		...
		mainTable.add(playButton).padTop(200).size(200,50);
		mainTable.row();
